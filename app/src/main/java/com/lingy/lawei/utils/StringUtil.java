...
public class StringUtil {
    ...
    public static String getWeiBoCreatedTime(Context context,String dateStr){
        SimpleDateFormat df = new SimpleDateFormat(WeiBo_ITEM_DATE_FORMAT, Locale.US);
        try {
            Date date = df.parse(dateStr);
            TimeUtils timeUtils = TimeUtils.instance(context);
            return timeUtils.buildTimeString(date.getTime()) + "   ";
